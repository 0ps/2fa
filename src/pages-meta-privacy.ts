...
export const PRIVACY_ROUTE = expandPage({
  ...
  h1En: "Privacy: secrets never leave this device",
  h1Zh: "隐私：密钥不离开这台设备",
  introEn: "This project is a static bundle. After the HTML, CSS, and JavaScript are downloaded, TOTP math does not need a backend. Setup keys may be kept in this browser’s localStorage only; they are never sent to a server.",
  introZh: "本项目是静态打包结果。下载 HTML、CSS 和 JavaScript 之后，TOTP 计算不需要后端。设置密钥只可能留在本机 localStorage，不会上传服务器。",
  bodyEn: "<p>This browser may write <code>totpPreferredLanguage</code> (language) and <code>totpLocalVault</code> (optional local history of names, setup keys, algorithm, digits, period, and clock offset). Generated codes stay in memory. Nothing is uploaded.</p><p>Query strings and path segments that contain a secret can appear in server access logs if you load this site from a host. Prefer <code>#secret=</code> fragments, which browsers do not send to the server. This UI may rewrite a query or path secret into a fragment after load.</p><p>QR scanning uses the browser BarcodeDetector API on a file you pick. The image is not uploaded.</p>",
  bodyZh: "<p>本浏览器可能写入 <code>totpPreferredLanguage</code>（语言）和 <code>totpLocalVault</code>（可选的本机历史：名称、设置密钥、算法、位数、周期和时间偏移）。验证码只留在内存。没有上传。</p><p>如果站点托管在服务器上，查询字符串和路径中的密钥可能出现在访问日志里。请优先使用浏览器不会发给服务器的 <code>#secret=</code> 片段。页面加载后可能把查询或路径中的密钥改写到片段里。</p><p>扫描二维码使用浏览器 BarcodeDetector，处理你选择的本地文件，图片不会上传。</p>",
  faqEn: [
