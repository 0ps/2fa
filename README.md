...
本分支是纯 HTML + JS，无构建步骤。把 `index.html`、`styles.css`、`app.js` 放到一起，双击打开，或丢到任意静态托管。截图用的是 RFC 6238 附录 B 的公开演示密钥，不是真实账号。
...
位数、周期、算法、时间校准收在「TOTP 选项」里。最后几秒会预告下一组码。

![SHA-256 与 TOTP 选项](docs/screenshots/sha256.png)

隐私说明：密钥只留在本机浏览器，不进 Cookie、不上传服务器。

![隐私](docs/screenshots/privacy.png)
...
1. **计算全在浏览器里。** 打开这三个静态文件即可。下载之后，Base32 解码、HMAC、动态截断都不需要网络。没有 API、没有 cookie 会话。
